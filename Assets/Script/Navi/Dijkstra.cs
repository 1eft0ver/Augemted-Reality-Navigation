...
public class Dijkstra : MonoBehaviour {
	...
	IEnumerator ShowAndHide( GameObject go, float delay ) {
        go.transform.Find("SelectDestStatus").gameObject.SetActive(true);
		string[] Splite;
		Splite = this.transform.parent.name.Split('_');
        GameObject.Find("DestStatusText").GetComponent<Text>().text = "已開始導航至" + Splite[1];
        yield return new WaitForSeconds(delay);
        go.transform.Find("SelectDestStatus").gameObject.SetActive(false);
    }
	public void createGraphAndNavi() {
		...
		StartCoroutine( ShowAndHide(GameObject.Find("MenuNavi"), 3.0f) );
